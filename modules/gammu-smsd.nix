...
let
  ...
  settingsFormat = pkgs.formats.ini {
    mkKeyValue =
      k: v: if v == null then "" else "${lib.strings.escape [ "=" ] k} = ${mkValueStringGammu v}";
  };
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.gammu-smsd = {
      ...
      preStart =
        lib.optionalString (cfgSrv == "files") (
      ...
      serviceConfig = self.data.systemdHarden // {
        ...
        StateDirectory = "%N";
        RuntimeDirectory = "%N";
        ExecStart = "${gammuPackage}/bin/gammu-smsd -c ${
          if cfg.pinFile != null then configFileRun else configFile
        }";
